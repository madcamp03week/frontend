...
// 파일 암호화 (시스템 키 사용)
export const encryptFileWithSystemKey = async (file: File): Promise<{ encryptedData: string; fileName: string }> => {
  try {
    const sodium = await initSodium();
    ...
    // nonce와 암호화된 데이터를 결합하여 base64로 인코딩
    const finalCombined = new Uint8Array(nonce.length + encrypted.length);
    finalCombined.set(nonce);
    finalCombined.set(encrypted, nonce.length);
    ...
    console.error('파일 시스템 키 암호화 오류:', error);
    throw new Error('파일 암호화에 실패했습니다.');
  }
};

// 파일 복호화 (시스템 키 사용)
export const decryptFileWithSystemKey = async (encryptedData: string): Promise<{ file: File; metadata: any }> => {
  try {
    const sodium = await initSodium();
    ...
    console.error('파일 시스템 키 복호화 오류:', error);
    throw new Error('파일 복호화에 실패했습니다.');
  }
};
